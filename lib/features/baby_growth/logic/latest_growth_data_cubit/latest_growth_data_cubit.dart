...
class LatestGrowthDataCubit extends Cubit<LatestGrowthDataState> {
  ...
  void latestGrowthData(String babyid) async {
    emit(const LatestGrowthDataState.loading());

    String token =
        await SharedPrefHelper.getSecuredString(SharedPrefKeys.userToken);
    final response =
        await _latestGrowthDataRepo.getLatestGrowthData(token, babyid);
    if (isClosed) return;
    ...
    response.when(
      success: (latestGrowthDataResponse) {
        ...
        emit(
          LatestGrowthDataState.success(
              latestGrowthDataResponse.data!),
        );
      },
